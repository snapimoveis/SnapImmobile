...
interface Props {
    devices: Device[]; // devices já é definido como array aqui
    onBlockDevice: (device: Device) => void;
}

export const DevicesTab: React.FC<Props> = ({ devices = [], onBlockDevice }) => { // Valor default []
    // Helper para filtrar com segurança
      ...
      <div className="py-8 animate-in fade-in">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              ...
              <div className="flex gap-2">
                  <span className="px-3 py-1 bg-[#623aa2] text-white text-xs font-medium rounded">Todos {devices.length}</span>
          ...
          <div className="bg-white dark:bg-black border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm transition-colors">
              <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-gray-400 text-xs uppercase">
                      <tr>
                          <th className="px-6 py-3 w-10"><input type="checkbox" className="rounded border-gray-300 text-purple-600 focus:ring-purple-500" /></th>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                      {devices.length === 0 ? (
                          <tr><td colSpan={7} className="text-center py-8 text-gray-500 dark:text-gray-400">Nenhum dispositivo registado.</td></tr>
                      ) : devices.map(device => (
                          <tr key={device.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                              <td className="px-6 py-4"><input type="checkbox" className="rounded border-gray-300 text-purple-600 focus:ring-purple-500" /></td>
                              ...
                              <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{device.userName || 'Desconhecido'}</td>
                              <td className="px-6 py-4 text-gray-500 dark:text-gray-400">
                                  {/* FIX: Casting explícito para number se necessário, ou valor padrão */}
                              ...
                              <td className="px-6 py-4">
                                  <button 
                                    ...
                                    className={`px-2 py-1 rounded text-[10px] font-bold uppercase hover:opacity-80 transition-opacity ${
                                      ...
                                      {(device.status === 'active' || device.status === 'Active') ? 'Acesso permitido' : 'Bloqueado'}
                                  </button>
                              </td>
