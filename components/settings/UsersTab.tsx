      ...
      <div className="py-8 animate-in fade-in space-y-6">
          ...
          <div className="flex justify-between items-center bg-white p-1 rounded-lg">
              <div className="flex gap-2">
                  <button className="px-3 py-1.5 bg-gray-600 text-white text-xs font-bold rounded flex items-center gap-2">Membros <span className="bg-white/20 px-1.5 rounded">{users.length}</span></button>
                  <button className="px-3 py-1.5 text-gray-500 hover:bg-gray-100 text-xs font-medium rounded flex items-center gap-2">Convites <span className="bg-gray-200 px-1.5 rounded text-gray-600">0</span></button>
              </div>
              <div className="flex gap-3">
                  <div className="relative">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input type="text" placeholder="Procurar utilizadores" className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm w-64 focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
              <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                      {users.length === 0 ? (
                          <tr><td colSpan={6} className="text-center py-12 text-gray-500">Nenhum utilizador encontrado.</td></tr>
                      ) : users.map(user => (
                          <tr key={user.id} className="hover:bg-gray-50 transition-colors group">
                              ...
                              <td className="px-6 py-4 text-gray-700 font-medium">
                                  {user.role === 'Administrador' ? 'Administrador' : 'Utilizador'}
                              </td>
                              ...
                              <td className="px-6 py-4 text-right text-gray-600 tabular-nums">
                                  {user.lastActive ? new Date(user.lastActive).toLocaleString('pt-PT') : '-'}
                              </td>
