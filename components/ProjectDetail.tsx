...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({ 
  ...
  const handleDownloadPhoto = async (e: React.MouseEvent, photo: Photo) => {
    e.stopPropagation(); // Impede abrir o editor ao clicar no download
    setDownloadingId(photo.id);

    try {
        // 1. Busca a imagem como "Blob" (Binário)
        const response = await fetch(photo.url, { mode: 'cors' });
        const blob = await response.blob();
        
        // 2. Cria um link temporário no navegador
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        
        // 3. Define o nome do ficheiro (força a extensão .jpg se faltar)
        const fileName = photo.name.toLowerCase().endsWith('.jpg') || photo.name.toLowerCase().endsWith('.png') 
            ? photo.name 
            : `${photo.name}.jpg`;
            
        link.download = fileName;
        
        // 4. Clica automaticamente e limpa
        ...
        // Fallback: Abre numa nova aba se o download direto falhar
                ...
                {displayPhotos.map((photo: Photo, index: number) => (
                  <React.Fragment key={photo.id || index}>
                    ...
                    >
                      ...
                      {/* Botões de Ação (Download corrigido) */}
