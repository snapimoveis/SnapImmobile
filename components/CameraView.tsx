...
export const CameraView: React.FC<CameraViewProps> = ({ onPhotoCaptured, onClose }) => {
  ...
  const playShutterSound = () => {
    try {
      ...
      // Synthetic "Clack" sound (fallback / complement)
  ...
  const capturePhotoSequence = async () => {
    ...
    // ------------------------------------------------------------
    // GET TRACK + CHECK EXPOSURE SUPPORT
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // REAL HP-HDR EV CURVE (+1.7EV a -3.5EV)
    // ------------------------------------------------------------
    ...
    // Fallback brightness (para browsers sem exposure real)
    ...
    try {
      ...
      for (let i = 0; i < 9; i++) {
        // ------------------------------------------------------------
        // EXPOSIÇÃO REAL (se o dispositivo suportar)
        // ------------------------------------------------------------
        if (supportsExposure && exposureIndexes[i] !== null) {
          try {
            ...
            await new Promise(r => setTimeout(r, 120)); // estabilizar
          } catch (err) {
            console.warn("ExposureCompensation failed:", err);
          }
        } else {
          // fallback artificial brightness
        ...
        previewsLocal.push({ url: shotUrl, ev: evLabels[i] });
      ...
      if (hdrMode) {
        ...
        try {
          // IA (Magic / Enhance) aplicada na melhor exposição
    ...
    <div className="fixed inset-0 bg-black z-50 font-sans overflow-hidden select-none">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          {hdrMode && (
            <span className={`text-[10px] font-bold text-yellow-500 tracking-widest absolute ${isLandscape ? 'bottom-20 -rotate-90' : 'top-16 right-6'}`}>
              HP-HDR
            </span>
          )}
        </div>
