      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-lg mb-8 relative">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
            <button onClick={handleAddContact} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
          ...
          <div key={contact.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all group relative">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[10px] uppercase font-bold px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">{contact.role}</span>
                <div className="mt-3 flex gap-2">
                  <a href={`tel:${contact.phone}`} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors"><Phone size={14} /></a>
                  {contact.email && <a href={`mailto:${contact.email}`} className="p-2 bg-gray-50 text-gray-600 rounded-lg hover:bg-gray-100 transition-colors"><Mail size={14} /></a>}
                  <a href={`https://wa.me/${contact.phone.replace(/[^0-9]/g, '')}`} target="_blank" className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors"><MessageCircle size={14} /></a>
                </div>
              </div>
            </div>
          </div>
